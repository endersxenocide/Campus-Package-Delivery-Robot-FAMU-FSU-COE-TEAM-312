...
//Waypoint Variables
         ...
         float WaypointX[] = {-30,10,-10,10,-10,10};
...
float theta = 0;   //Initial Direction Robot is facing
float last_theta = 0;

float WheelV_Xpos = 0;
float WheelV_Ypos = 0;
float WheelV_ThetaPOS = WheelV_ThetaPOS;
...
float LatOrgin = 30.421950;
float LongOrgin = -84.319614;
...
k32 = W;
    ...
    //Correct heading based on current heading and desired heading
    ...
    Error_Integral = Error_Integral + 0.02 * Error_Angle;
    ...
    Velocity_Error = (Desired_Velocity - V);
    Operating_Speed = -600*Desired_Velocity - (600 * Velocity_Error); //Ramp velocity as higher value until desired velocity reached
...
//
//
//    Serial.print("Speed Commmand: ");
//    Serial.println(Operating_Speed);



    //P controller
    //isteer = -800*Error_Angle;   //Worked well while stopped


//Worked well while moving as just P controller
    //isteer = -200 * Error_Angle;


    K_Integral = 20;

    isteer = -200*Error_Angle - K_Integral * Error_Integral;
...
//Set for turning when stopped
//    if (isteer < -1 && isteer > -250)
//    {
//      isteer = isteer - 250;
//    }
//    if (isteer > 1 && isteer < 250)
...
//Set for turning when moving
    if (isteer < -1 && isteer > -100)
    {
      isteer = isteer - 100;
    }
    if (isteer > 1 && isteer < 100)
    ...
    Send(Operating_Speed*estop*endstop, isteer*estop*endstop);
